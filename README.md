...
| [docs/Besm6_Intrinsics.md](docs/Besm6_Intrinsics.md) | BESM-6 compiler intrinsics (`<besm6.h>`): user manual for the twelve `__besm6_*` intrinsics — the supervisor instructions `ext`/`mod`, the mode-word trio `getpsw`/`setpsw`/`maskpsw`, the bit-manipulation instructions with no C equivalent, `stop` and the extracode trap |
