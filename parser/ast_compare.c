...
bool compare_unary_op(const UnaryOp *a, const UnaryOp *b);
...
bool compare_expr(const Expr *a, const Expr *b)
{
    ...
    case EXPR_UNARY_OP:
        if (!compare_unary_op(a->u.unary_op.op, b->u.unary_op.op))
            return false;
    ...
    case EXPR_ASSIGN:
        if (!compare_expr(a->u.assign.target, b->u.assign.target))
            return false;
        if (!compare_assign_op(a->u.assign.op, b->u.assign.op))
            return false;
