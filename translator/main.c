...
void process_file(const Args *args)
{
    ...
    Tac_Program *program = translate(input_file);
    fclose(input_file);
    ...
    case FORMAT_TAC:
        ...
        if (args->debug) {
            print_tac_program(stdout, program);
        }
        //TODO: export_tac(fileno(output_file), program);
        break;
    case FORMAT_YAML:
        if (args->verbose) {
            printf("Emitting YAML format to %s\n", args->output_file);
        }
        //TODO: export_yaml(output_file, program);
        break;
    case FORMAT_DOT:
        if (args->verbose) {
            printf("Emitting Graphviz DOT script to %s\n", args->output_file);
        }
        //TODO: export_dot(output_file, program);
        break;
    }
