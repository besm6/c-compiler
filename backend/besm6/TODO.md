...
| 24a | Array-parameter adjustment (array→pointer decay) | Adjust function array params to pointer type so `int a[2]`≡`int *a` and `int a[5][3]`≡`int (*a)[3]` prototypes match. Re-enables ch15 ArrayAsArgument (chapter15_tests.cpp). Also a prerequisite for the adjusted-param sub-call in 24d. |
