...
| 54 | Struct-by-value parameter / return ABI | Passing a struct by value as a parameter, and returning multi-word / in-memory-class structs, is unimplemented (only simple ≤3-word struct *returns* work today — `StructByValueReturnRun`/`StructByValueReturnThreeWordRun`). Implement the BESM-6 struct-by-value calling convention (copy aggregate args into the callee frame; large/MEMORY-class structs via a hidden pointer; sret for in-memory returns) and re-enable the ch18 calling-convention/stack-clobber cohort, all of which already have `strcmp`/`strncmp`/`exit` available: ParametersStackClobber, ParamsAndReturnsStackClobber, ClassifyParams, ParamCallingConventions, ReturnCallingConventions; StructSizes (passes structs of every size 1..24 by value) and RetvalStructSizes (returns them by value); also StructCopyStackClobber (additionally needs a no-shadowing rename). The packed sub-word char layout these need is now in place (former #42). (6–7) Each test's `DISABLED_` comment names this blocker. |
| 55 | Struct-init representation codegen (mixed-type aggregate initializers) | Aggregate initialization mixing `long`, `double`, and `unsigned char[]` members validates wrong or errors at run time (the values round-trip incorrectly through BESM-6 representation, overlapping char-signedness #44 and FP-member packing). Fix the static/automatic struct-init lowering for mixed-type members and re-enable the ch18 initializer tests (validation is by-pointer, `strcmp` available): NestedAutoStructInitializers (returns 1), NestedStaticStructInitializers (run error), StaticStructInitializers (returns 2) (3). Each test's `DISABLED_` comment names this blocker. |
