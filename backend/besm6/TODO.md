...
| #  | Task | Description |
|----|------|-------------|
| U1 ✅ | Unix emitter — code + framing | **Done.** `emit_unix.c` `emit_unix_module`: segments `.text`/`.data`/`.bss` (string constants fold into `.data`; `.strng` unused); AT&T line format `[modreg] mnem addr` (leading modreg per docs/Besm6_Unix_Assembler.md §3/§8); reuses `besm_latin_mnem` + `besm_operand_shape`. Unix name sanitizer (allow `_ . $`; map `%`→`.`, `/`→`$`; no 8-char truncation). Constant formatter via the `#`-pool (octal `#0NNN`, real `#<val>`). Externals/relocation **translated**: `SUBP`→drop (b6as auto-externs undefined names), `NAME`/`ENTRY`→`.globl name`+label, `BASE`/`END`→drop; `Z00` split by pointer kind — plain pointer pair → `.word name+off`, fat pointer → raw `@00` pair (marker on the leading modreg). Runtime-helper names are canonical `$` (`b$ret`, `b$save`), so they pass through unchanged (Madlen lowers `$`→`/`); hardcoded backend `b/…` literals were switched to `b$…` to make the IR form uniform. Wired into `besm_emit_module`. Golden `.s` tests in `test/unix_tests.cpp` (inline `R"(...)"`, per repo convention) cover scalar, call, global, float const, static array, string, plain/fat pointer. |
