...
| #  | Task | Description |
|----|------|-------------|
| U2 | Unix `libc.a` + b6as/b6ld/b6ar wiring | Compile the reusable `LIBC_C_PORTABLE` set through `genbesm --unix` → `b6as` → archive with `b6ar` + `b6ranlib` into `libc.a`; add a `libc/besm6/unix/` sibling supplying the three Dubna leaves (`putbyte`/`flush`/`getch`) and the `b_*` helpers reimplemented in `b6as` syntax. The Unix emitter renders runtime-helper names with `$` (`b$ret`, `b$save`, …), so the sibling's symbols must match. Add CMake to locate the sibling `v7besm/cmd/{as,ld,ar,ranlib}` binaries on `PATH` (like `dubna` today). *Acceptance:* `libc.a` builds; `b6as` assembles every libc `.s` cleanly; `b6ld` links a trivial program object against it into a `b.out`. |
| U2a | Fix `b6ranlib` (v7besm) | `b6ranlib` corrupts any `b6ar` archive of **≥ 6 members**: `b6ld` then rejects it with `bad magic` (archives of ≤ 5 members index fine; an un-indexed archive always links). Minimal repro: assemble six one-line objects (`.globl sN` / `sN:` / `13 uj`), `b6ar rc a.a s1.o … s6.o`, `b6ranlib a.a`, then `b6ld -o /dev/null m.o a.a`. Likely an offset/size miscomputation in the `__.SYMDEF` writer (`cmd/ranlib/ranlib.c` `fixsize`, `W = 6` word rounding) or the archive-member reader in `b6ld`. Until fixed, `libc/besm6/unix/CMakeLists.txt` builds `libc.a` with `b6ar` **only** (no `b6ranlib`); `b6ld` reads member symbol tables directly, so linking still works (order-sensitive for back-references). Lives in the sibling `v7besm` repo, tracked here because it blocks a symbol-indexed Unix `libc.a`. |
