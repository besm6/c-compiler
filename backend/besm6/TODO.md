...
|---|------|-------------|--------|
| 12 | Backend directory and build | Create `backend/besm6/` with `CMakeLists.txt` producing the `gen` executable (reads binary TAC, writes Madlen `.mad`). Wire into top-level `CMakeLists.txt` and `Makefile` alongside `parse` and `lower`. | S |
