...
### `b/save` — [b_save.madlen](../backend/besm6/madlen/b_save.madlen)
...
### `b/save0` — [b_save0.madlen](../backend/besm6/madlen/b_save0.madlen)
...
### `b/ret` — [b_ret.madlen](../backend/besm6/madlen/b_ret.madlen)
...
### `b/true` — [b_true.madlen](../backend/besm6/madlen/b_true.madlen)
...
#### `b/mul` — [b_mul.madlen](../backend/besm6/madlen/b_mul.madlen)
...
#### `b/div` — [b_div.madlen](../backend/besm6/madlen/b_div.madlen)
...
#### `b/mod` — [b_mod.madlen](../backend/besm6/madlen/b_mod.madlen)
...
#### `b/uadd` — [b_uadd.madlen](../backend/besm6/madlen/b_uadd.madlen) — `a + b` (unsigned)
...
#### `b/usub` — [b_usub.madlen](../backend/besm6/madlen/b_usub.madlen) — `a − b` (unsigned)
...
#### `b/umul` — [b_umul.madlen](../backend/besm6/madlen/b_umul.madlen) — `a * b` (unsigned)
...
#### `b/udiv` — [b_udiv.madlen](../backend/besm6/madlen/b_udiv.madlen) — `a / b` (unsigned)
...
#### `b/umod` — [b_umod.madlen](../backend/besm6/madlen/b_umod.madlen) — `a % b` (unsigned)
...
constant from [b_true.madlen](../backend/besm6/madlen/b_true.madlen).
...
|---------|--------|------|-----------------|--------|-------------------|
| `b/not` | [b_not.madlen](../backend/besm6/madlen/b_not.madlen) | `!b` | `aex` → A = b XOR 0 = b | Logical | `uza`: A = 0 (b = 0) |
| `b/eq` | [b_eq.madlen](../backend/besm6/madlen/b_eq.madlen) | `a == b` | `15 ,aex,` → A = b XOR a | Logical | `uza`: A = 0 (a = b) |
| `b/ne` | [b_ne.madlen](../backend/besm6/madlen/b_ne.madlen) | `a != b` | `15 ,aex,` → A = b XOR a | Logical | `u1a`: A ≠ 0 (a ≠ b) |
| `b/lt` | [b_lt.madlen](../backend/besm6/madlen/b_lt.madlen) | `a < b` | `15 ,x-a,` → A = a − b | Additive | `u1a`: A < 0 (a < b) |
| `b/le` | [b_le.madlen](../backend/besm6/madlen/b_le.madlen) | `a <= b` | `15 ,a-x,` → A = b − a | Additive | `uza`: A ≥ 0 (b ≥ a) |
| `b/gt` | [b_gt.madlen](../backend/besm6/madlen/b_gt.madlen) | `a > b` | `15 ,a-x,` → A = b − a | Additive | `u1a`: A < 0 (b < a) |
| `b/ge` | [b_ge.madlen](../backend/besm6/madlen/b_ge.madlen) | `a >= b` | `15 ,x-a,` → A = a − b | Additive | `uza`: A ≥ 0 (a ≥ b) |
...
#### `b/ult` — [b_ult.madlen](../backend/besm6/madlen/b_ult.madlen) — `a < b` (unsigned)

Packs `a ⊕ b` under `b` and extracts bit 48 — the borrow of `a − b`, i.e. `a < b`.

#### `b/ule` — [b_ule.madlen](../backend/besm6/madlen/b_ule.madlen) — `a <= b` (unsigned)

Packs `a ⊕ b` under `a` and extracts bit 48 (= `a > b`), then inverts it against `b/true`.

#### `b/ugt` — [b_ugt.madlen](../backend/besm6/madlen/b_ugt.madlen) — `a > b` (unsigned)

Packs `a ⊕ b` under `a` and extracts bit 48 — the borrow of `b − a`, i.e. `a > b`.

#### `b/uge` — [b_uge.madlen](../backend/besm6/madlen/b_uge.madlen) — `a >= b` (unsigned)
...
|---------|--------|-----------|-------------|-------|----------------|
| `b/flt` | [b_flt.madlen](../backend/besm6/madlen/b_flt.madlen) | `a < b` | `15 ,x-a,` → A = a − b | Additive | `u1a`: A < 0 (a < b) |
| `b/fle` | [b_fle.madlen](../backend/besm6/madlen/b_fle.madlen) | `a <= b` | `15 ,a-x,` → A = b − a | Additive | `uza`: A ≥ 0 (b ≥ a) |
| `b/fgt` | [b_fgt.madlen](../backend/besm6/madlen/b_fgt.madlen) | `a > b` | `15 ,a-x,` → A = b − a | Additive | `u1a`: A < 0 (b < a) |
| `b/fge` | [b_fge.madlen](../backend/besm6/madlen/b_fge.madlen) | `a >= b` | `15 ,x-a,` → A = a − b | Additive | `uza`: A ≥ 0 (a ≥ b) |
...
#### `b/dtoi` — [b_dtoi.madlen](../backend/besm6/madlen/b_dtoi.madlen) — `double` → signed `int`
...
#### `b/dtou` — [b_dtou.madlen](../backend/besm6/madlen/b_dtou.madlen) — `double` → unsigned `int`
...
#### `b/utod` — [b_utod.madlen](../backend/besm6/madlen/b_utod.madlen) — unsigned `int` → `double`
...
#### `b/stb` — [b_stb.madlen](../backend/besm6/madlen/b_stb.madlen) — store one byte
...
#### `b/pinc` / `b/pdec` — [b_pinc.madlen](../backend/besm6/madlen/b_pinc.madlen) / [b_pdec.madlen](../backend/besm6/madlen/b_pdec.madlen) — `char*`++ / `char*`--
...
#### `b/padd` — [b_padd.madlen](../backend/besm6/madlen/b_padd.madlen) — fat pointer + signed byte count
...
#### `b/pdiff` — [b_pdiff.madlen](../backend/besm6/madlen/b_pdiff.madlen) — fat pointer − fat pointer
...
#### `b/tout` — [b_tout.madlen](../backend/besm6/madlen/b_tout.madlen)
